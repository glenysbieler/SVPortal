<?php
/**
 * all_estates.php — "All Estates" tool (Administration)
 *
 * The grid-wide counterpart to "My Estates" (regions.php): instead of
 * listing only the estates the logged-in user owns or manages, this lists
 * EVERY estate on the grid, regardless of ownership. Same per-estate
 * frame/region-grid layout and same region detail modal (status, stats,
 * restart, broadcast, maturity — all via the existing
 * user_can_manage_region_maturity() gate, includes/helpers.php) and the
 * same Estate Tools modal (manager roster) — those already grant full
 * access to anyone meeting the 'Grid Staff' tier, so they are reused
 * largely unchanged here. The Console button on the region modal remains
 * Administrator-tier only, same as everywhere else it appears.
 *
 * The one tool unique to this page is "Change Owner" — folded INTO the
 * Estate Tools modal next to the current owner's name, not a separate
 * button — which lets an Administrator either take ownership of an estate
 * themselves or transfer it to any resident, via a direct write to
 * estate_settings.EstateOwner (estate_tools.php's change_owner action; see
 * ENABLE_ESTATE_OWNER_TRANSFER in config.php and
 * user_can_change_estate_owner() in includes/helpers.php). This is
 * deliberately the ONE piece of this page that stays Administrator-only —
 * everything else here matches the Grid Staff tier used by the rest of the
 * Administration submenu.
 *
 * ── Why this tool needs to exist at all ────────────────────────────────────
 * OpenSim does NOT give Administrators ("Grid Gods") implicit access to
 * estates they don't own or manage — estate_settings.EstateOwner and
 * estate_managers are the only access records, and UserLevel doesn't factor
 * in at the simulator level. This page is what gives the portal's Grid
 * Staff/Administrator tiers a working equivalent of that access, entirely
 * within the portal's own permission model (every read/write here is still
 * gated by user_level_meets() — see Hard Rule 9, CLAUDE.md).
 *
 * ── Access — per-feature tier breakdown ────────────────────────────────────
 * This page does NOT use a single uniform tier; each capability is gated
 * separately, matching the existing function each one already delegates to:
 *   - Page access (seeing this page / drawer item at all): 'Grid Staff' —
 *     see $access_denied below.
 *   - Region modal: status, stats, restart, broadcast, maturity: 'Grid
 *     Staff' — user_can_manage_region_maturity(), same function
 *     change_maturity.php / region_status.php / region_stats_data.php /
 *     region_restart.php / region_broadcast.php all already use.
 *   - Region modal: Console button: 'Administrator' — unchanged, same gate
 *     used on regions.php's region modal.
 *   - Estate Tools modal: view roster, add/remove manager: 'Grid Staff' —
 *     user_can_view_estate_tools() / user_can_manage_estate_managers().
 *   - Estate Tools modal: Change Owner (Take Ownership / Give Ownership):
 *     'Administrator' ONLY — user_can_change_estate_owner(). Never the
 *     estate's own owner or managers, since this action redefines who that
 *     is — see that function's docblock.
 * Unlike regions.php, there is no "no access -> redirect" case for page
 * access: visibility is purely UserLevel-based (the drawer item is hidden
 * below the Grid Staff tier), and a user without that tier landing here
 * directly sees an access-denied message rather than being redirected —
 * matching console.php's pattern.
 *
 * ── Data source ───────────────────────────────────────────────────────────
 * get_all_estates() (includes/estates.php) — every EstateID on the grid via
 * get_estate_block_data(), with NO ownership filtering (unlike
 * get_estates_for_user(), used by regions.php). Ordered by EstateName; no
 * owner/manager grouping since that grouping is meaningless for a viewer
 * seeing every estate — the header badge shows the current owner's name
 * directly instead (see the estate-role-badge markup below).
 *
 * Session: real authentication via includes/auth.php
 * Data:    SELECT only — estate_settings, estate_managers, estate_map, regions
 *          (plus the documented estate_settings.EstateOwner write exception
 *          for Change Owner — see ENABLE_ESTATE_OWNER_TRANSFER, config.php)
 */
...
// ─── Access control ─────────────────────────────────────────────────────────
// Page-level gate is 'Grid Staff' — see the docblock's per-feature
// breakdown above for why individual actions within the page (Console,
// Change Owner) are gated more strictly at the point of use rather than
// here. Unlike regions.php (redirects to profile.php), a user without
// access landing here directly sees an access-denied message in-page —
// matching console.php's pattern, since this is a drawer item hidden below
// the Grid Staff tier and so should only be reached this way via a stale
// bookmark/link.
$access_denied = !user_level_meets($userlevel, 'Grid Staff');

$estates = [];
if (!$access_denied) {
    $estates = get_all_estates($db);
}

// Resolve each estate's owner display name once (used in the region detail
// modal and the Estate Tools modal's Change Owner section). NULL_UUID
// owners are shown as "Unowned" — unlike regions.php, this DOES occur here
// in practice (any estate with no current owner is still listed, since
// this page shows every estate regardless of ownership state).
foreach ($estates as &$estate) {
    if ($estate['owner_uuid'] === NULL_UUID) {
...
<head>
    ...
    <title>All Estates — <?= htmlspecialchars(GRID_NAME) ?></title>
    <?php render_shared_css(); ?>
    <script>const PORTAL_CSRF = <?= json_encode($csrf_token) ?>;</script>
    <style>
        .all-estates-feature-disabled {
            font-size: 0.9rem;
        ...
        /* Owner line: name + inline "Change Owner" button, Administrator-tier
           only (toggled via JS based on canChangeOwner — see loadEstateManagers()). */
        .estate-tools-owner-line {
            display: flex;
            align-items: center;
            flex-wrap: wrap;
            gap: 10px;
        ...
        /* Same reasoning as #removeManagerModal above — #takeOwnershipModal
           is opened from the Change Owner section INSIDE the Estate Tools
           modal and must stack above it. */
        #takeOwnershipModal {
            z-index: 600;
        }
...
<?php render_drawer('all_estates', [], $userlevel, $has_estate_access); ?>

<main class="page-wrap" id="main-content">

    <div class="estates-header-panel">
        <h1 class="panel-heading">All Estates</h1>
        <p class="panel-subhead">Every estate on the grid, regardless of ownership.</p>
    </div>
    ...
    <?php elseif (empty($estates)): ?>
    <div class="estates-empty">
        There are no estates on this grid yet.
    </div>
    <?php else: ?>
        <?php foreach ($estates as $estate): ?>
        <section class="estate-block" data-estate-id="<?= (int)$estate['estate_id'] ?>" aria-label="<?= htmlspecialchars($estate['name'], ENT_QUOTES) ?>">
            <div class="estate-block-header">
                <h2 class="estate-name"><?= htmlspecialchars($estate['name']) ?></h2>
                <span class="estate-role-badge <?= $estate['owner_uuid'] === NULL_UUID ? 'manager' : 'owner' ?>">
                    <?= htmlspecialchars($estate['owner_name']) ?>
                </span>
            ...
            <div class="estate-block-body">
                ...
                <div class="region-grid">
                    <?php
                        // canManageMaturity is a foregone conclusion here, same as
                        // regions.php: page access to all_estates.php is already gated
                        // to the 'Grid Staff' tier (see $access_denied above), and
                        // user_can_manage_region_maturity() grants maturity-change
                        // access to that exact same tier — so every viewer who can
                        // reach this page at all already passes it, regardless of
                        // whether they own/manage any particular estate shown. Real
                        // enforcement either way happens server-side in
                        // change_maturity.php; this only controls whether the pill in
                        // the UI invites a click.
                        $can_manage_maturity = os_write_feature_enabled('ENABLE_CHANGE_MATURITY');
                    ?>
                    ...
                    <?php
                        $region_modal_json = json_encode([
                            ...
                            'canManageMaturity'  => $can_manage_maturity,
...
<div class="region-modal-overlay" id="estateToolsModalOverlay"
    ...
    <div class="region-modal estate-tools-modal">
        ...
        <div class="region-modal-body estate-tools-modal-body">
            <p class="estate-tools-detail-line estate-tools-owner-line">
                <span class="label">Owner:</span> <span id="estateToolsOwnerName">—</span>
                <button type="button" class="btn-primary estate-tools-change-owner-btn"
                        id="estateToolsChangeOwnerBtn" style="display:none;"
                        onclick="toggleChangeOwnerSection()">
                    Change Owner
                </button>
            </p>

            <!-- Change Owner — folded into this modal (not a separate one):
                 only ever shown to users meeting the 'Administrator' tier
                 (see canChangeOwner in estate_tools.php's details response /
                 user_can_change_estate_owner() in includes/helpers.php),
                 toggled open by the button above. -->
            <div id="changeOwnerSection" style="display:none;">
                <h3 class="estate-tools-section-heading">Take Ownership</h3>
                <p class="estate-tools-empty" style="margin-bottom: 10px;">
                    Make yourself the owner of this estate.
                </p>
                <button type="button" class="btn-primary" id="takeOwnershipBtn" onclick="openTakeOwnershipConfirm()">
                    Take Ownership
                </button>

                <h3 class="estate-tools-section-heading">Give Ownership to Someone Else</h3>
                <div class="estate-tools-add-wrap" id="changeOwnerSearchWrap">
                    <input type="text"
                           ...
                           autocomplete="off"
                           spellcheck="false"
                           aria-label="Search residents to give estate ownership to"
                           aria-autocomplete="list"
                           aria-controls="changeOwnerSearchResults">
                    <div class="estate-tools-add-results" id="changeOwnerSearchResults" role="listbox" aria-label="Search results"></div>
                </div>
                <div id="changeOwnerSelectedArea" class="estate-tools-manager-row" style="display:none; margin-top: 12px;">
                    <span class="estate-tools-manager-name" id="changeOwnerSelectedName"></span>
                    <button type="button" class="btn-primary" id="giveOwnershipBtn" onclick="doChangeOwner()">
                        Give Ownership
                    </button>
                </div>
                <p class="estate-tools-add-status" id="changeOwnerStatus"></p>
            </div>
...
<div class="dissolve-modal-overlay" id="takeOwnershipModal"
     role="dialog" aria-modal="true"
     aria-labelledby="takeOwnershipModalTitle" aria-hidden="true">
    <div class="dissolve-modal-card">
        <h2 class="dissolve-modal-title" id="takeOwnershipModalTitle">Take ownership of this estate?</h2>
        <p class="dissolve-modal-body">
            This will make <strong id="takeOwnershipYourName"></strong> the owner of
            <strong id="takeOwnershipEstateName"></strong>, replacing
            <strong id="takeOwnershipCurrentName"></strong>. Are you sure?
        </p>
        <p class="dissolve-modal-status" id="takeOwnershipStatus"></p>
        <div class="dissolve-modal-actions">
            <button class="btn-cancel" id="takeOwnershipCancelBtn" onclick="closeTakeOwnershipModal()">
                Cancel
            </button>
            <button class="btn-primary-pill-destructive" id="takeOwnershipConfirmBtn" onclick="doTakeOwnership()">
                Yes, take ownership
            </button>
        </div>
    </div>
...
function openEstateToolsModal(estate) {
    ...
    // Change Owner — hidden until loadEstateManagers() resolves canChangeOwner;
    // reset to closed/empty on every open so a previous estate's in-progress
    // search/selection never leaks into a newly opened one.
    document.getElementById('estateToolsChangeOwnerBtn').style.display = 'none';
    closeChangeOwnerSection();
...
function loadEstateManagers(estateId) {
    ...
    fetch('estate_tools.php?' + new URLSearchParams({
        ...
        .then(function(data) {
            ...
            estateToolsData.canChangeOwner = data.canChangeOwner;
            estateToolsData.ownerName      = data.estate.owner_name;
            estateToolsData.ownerUuid      = data.estate.owner_uuid;
            document.getElementById('estateToolsOwnerName').textContent = data.estate.owner_name;

            document.getElementById('estateToolsChangeOwnerBtn').style.display = data.canChangeOwner ? '' : 'none';
...
/* ── Change Owner section (inline within the Estate Tools modal) ───────── */
// Unlike a separate modal, this operates directly on estateToolsData (the
// Estate Tools modal's own state) rather than tracking its own estate —
// the section only ever exists while that modal is open, so there is no
// separate "which estate" to track. The same "ignore stale async
// responses" guard (checking estateToolsData.estateId) still applies.
let changeOwnerSelectedTarget  = null; // { uuid, fullname } once picked from search
let changeOwnerSearchDebounce  = null;
let changeOwnerSearchLastQuery = '';

const changeOwnerSearchInputEl   = document.getElementById('changeOwnerSearchInput');
const changeOwnerSearchResultsEl = document.getElementById('changeOwnerSearchResults');

function toggleChangeOwnerSection() {
    const section = document.getElementById('changeOwnerSection');
    if (section.style.display === 'none') {
        openChangeOwnerSection();
    } else {
        closeChangeOwnerSection();
    }
}

function openChangeOwnerSection() {
    changeOwnerSelectedTarget = null;
    document.getElementById('changeOwnerStatus').textContent = '';
    document.getElementById('takeOwnershipBtn').disabled     = false;
    document.getElementById('takeOwnershipBtn').textContent  = 'Take Ownership';
    changeOwnerSearchInputEl.value = '';
    closeChangeOwnerSearchResults();
    document.getElementById('changeOwnerSelectedArea').style.display = 'none';
    document.getElementById('changeOwnerSection').style.display = '';
}

function closeChangeOwnerSection() {
    document.getElementById('changeOwnerSection').style.display = 'none';
    changeOwnerSelectedTarget = null;
    closeChangeOwnerSearchResults();
    changeOwnerSearchInputEl.value = '';
    document.getElementById('changeOwnerStatus').textContent = '';
...
/* ── Give Ownership: typeahead — same pattern as the Estate Tools "Add a
       Manager" search above, just targeting a different endpoint action
       and a single-select outcome instead of an immediate add. ────────── */
changeOwnerSearchInputEl.addEventListener('input', function () {
    const q = this.value.trim();
    clearTimeout(changeOwnerSearchDebounce);

    if (q.length < 2) {
        closeChangeOwnerSearchResults();
        return;
    }
    if (q === changeOwnerSearchLastQuery) return;

    changeOwnerSearchDebounce = setTimeout(function() { doChangeOwnerSearch(q); }, 280);
});

changeOwnerSearchInputEl.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') { closeChangeOwnerSearchResults(); return; }
    if (e.key === 'ArrowDown') {
        e.preventDefault();
        const first = changeOwnerSearchResultsEl.querySelector('.estate-tools-add-result-item');
        if (first) first.focus();
    }
});

changeOwnerSearchResultsEl.addEventListener('keydown', function (e) {
    const items = [...changeOwnerSearchResultsEl.querySelectorAll('.estate-tools-add-result-item')];
    const idx   = items.indexOf(document.activeElement);
    if (e.key === 'ArrowDown' && idx < items.length - 1) {
        e.preventDefault(); items[idx + 1].focus();
    } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        if (idx <= 0) changeOwnerSearchInputEl.focus();
        else items[idx - 1].focus();
    } else if (e.key === 'Escape') {
        closeChangeOwnerSearchResults(); changeOwnerSearchInputEl.focus();
    }
});

// Delegated click handling — see renderEstateToolsAddResults()'s comment
// (above, in the Estate Tools section) for why data-* attributes are used
// rather than onclick="..." strings built with embedded names.
changeOwnerSearchResultsEl.addEventListener('click', function (e) {
    const btn = e.target.closest('.estate-tools-add-result-item');
    if (!btn) return;
    selectChangeOwnerTarget(btn.dataset.uuid, btn.dataset.fullname);
});

document.addEventListener('click', function (e) {
    const wrap = document.getElementById('changeOwnerSearchWrap');
    if (wrap && !wrap.contains(e.target)) {
        closeChangeOwnerSearchResults();
    }
});

function doChangeOwnerSearch(q) {
    if (!estateToolsData) return;
    changeOwnerSearchLastQuery = q;
    changeOwnerSearchResultsEl.innerHTML = '<div class="estate-tools-add-results-searching">Searching…</div>';
    changeOwnerSearchResultsEl.classList.add('open');

    fetch('estate_tools.php?' + new URLSearchParams({
        action: 'search_residents_for_owner', estate_id: String(estateToolsData.estateId), q: q, csrf: PORTAL_CSRF,
    }))
        .then(function(r) { return r.json(); })
        .then(function(data) {
            if (!data.ok) { showChangeOwnerSearchError(data.error || 'Search failed.'); return; }
            renderChangeOwnerSearchResults(data.results, q);
        })
        .catch(function() { showChangeOwnerSearchError('Search failed. Please try again.'); });
}

function renderChangeOwnerSearchResults(items, q) {
    if (items.length === 0) {
        changeOwnerSearchResultsEl.innerHTML = '<div class="estate-tools-add-results-empty">No matching residents found for "' + escapeHtml(q) + '"</div>';
        return;
    }
    changeOwnerSearchResultsEl.innerHTML = items.map(function(r) {
        return '<button type="button" class="estate-tools-add-result-item" ' +
            'data-uuid="' + escapeHtml(r.uuid) + '" data-fullname="' + escapeHtml(r.fullname) + '">' +
            escapeHtml(r.fullname) +
            '</button>';
...
function showChangeOwnerSearchError(msg) {
    changeOwnerSearchResultsEl.innerHTML = '<div class="estate-tools-add-results-empty">' + escapeHtml(msg) + '</div>';
}

function closeChangeOwnerSearchResults() {
    changeOwnerSearchResultsEl.classList.remove('open');
    changeOwnerSearchResultsEl.innerHTML = '';
    changeOwnerSearchLastQuery = '';
}

function selectChangeOwnerTarget(uuid, fullname) {
    changeOwnerSelectedTarget = { uuid: uuid, fullname: fullname };
    closeChangeOwnerSearchResults();
    changeOwnerSearchInputEl.value = '';
    document.getElementById('changeOwnerSelectedName').textContent = fullname;
    document.getElementById('changeOwnerSelectedArea').style.display = '';
    document.getElementById('changeOwnerStatus').textContent = '';
}

function doChangeOwner() {
    if (!estateToolsData || !changeOwnerSelectedTarget) return;
    const estateId = estateToolsData.estateId;
    const target    = changeOwnerSelectedTarget;
    const btn       = document.getElementById('giveOwnershipBtn');
    const status    = document.getElementById('changeOwnerStatus');

    btn.disabled = true;
    status.textContent = 'Giving ownership to ' + target.fullname + '…';

    fetch('estate_tools.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
            action: 'change_owner', estate_id: String(estateId), uuid: target.uuid, csrf: PORTAL_CSRF,
        }),
    })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            ...
            if (!data.ok) {
                status.textContent = data.error || 'Could not change the estate owner — please try again.';
                return;
            }

            estateToolsData.ownerName = data.ownerName;
            estateToolsData.ownerUuid = data.ownerUuid;
            changeOwnerSelectedTarget = null;
            document.getElementById('estateToolsOwnerName').textContent = data.ownerName;
            document.getElementById('changeOwnerSelectedArea').style.display = 'none';
            status.textContent = data.ownerName + ' is now the owner of this estate.';

            // Update the estate block's badge behind the modal so the page
            // stays in sync without a full reload.
            updateEstateOwnerBadge(estateId, data.ownerName, data.ownerUuid);
        })
        .catch(function() {
            if (!estateToolsData || estateToolsData.estateId !== estateId) return;
            btn.disabled = false;
            status.textContent = 'Could not change the estate owner — please try again.';
        });
}

/* ── Take Ownership: confirm modal ──────────────────────────────────────── */
function openTakeOwnershipConfirm() {
    if (!estateToolsData) return;

    document.getElementById('takeOwnershipYourName').textContent     = <?= json_encode($full_name === '' ? 'You' : strip_tags($full_name)) ?>;
    document.getElementById('takeOwnershipEstateName').textContent   = estateToolsData.name;
    document.getElementById('takeOwnershipCurrentName').textContent  = estateToolsData.ownerName || '—';
    document.getElementById('takeOwnershipStatus').textContent       = '';

    const o = document.getElementById('takeOwnershipModal');
    o.classList.add('open');
    o.setAttribute('aria-hidden', 'false');
}

function closeTakeOwnershipModal() {
    const o = document.getElementById('takeOwnershipModal');
    o.classList.remove('open');
    o.setAttribute('aria-hidden', 'true');
}
document.getElementById('takeOwnershipModal').addEventListener('click', function(e) {
    if (e.target === this) closeTakeOwnershipModal();
});

function doTakeOwnership() {
    if (!estateToolsData) return;
    const estateId   = estateToolsData.estateId;
    const confirmBtn = document.getElementById('takeOwnershipConfirmBtn');
    const status     = document.getElementById('takeOwnershipStatus');

    confirmBtn.disabled = true;
    status.textContent  = 'Taking ownership…';

    fetch('estate_tools.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
            action: 'change_owner', estate_id: String(estateId), take_ownership: '1', csrf: PORTAL_CSRF,
        }),
    })
        .then(function(r) { return r.json(); })
        .then(function(data) {
            ...
            if (!data.ok) {
                status.textContent = data.error || 'Could not take ownership — please try again.';
                return;
            }

            closeTakeOwnershipModal();

            if (estateToolsData && estateToolsData.estateId === estateId) {
                estateToolsData.ownerName = data.ownerName;
                estateToolsData.ownerUuid = data.ownerUuid;
                document.getElementById('estateToolsOwnerName').textContent = data.ownerName;
            }
        ...
        .catch(function() {
            confirmBtn.disabled = false;
            status.textContent = 'Could not take ownership — please try again.';
        });
}

/**
 * Update the owner-name badge on an estate block's header (behind any open
 * modal) after a successful Change Owner / Take Ownership action, so the
 * page reflects the new owner without a full reload. Matched via the
 * block's data-estate-id attribute (set in the PHP loop above) rather than
 * parsing onclick attribute strings — same data-* lookup convention used
 * throughout this file (e.g. the manager remove buttons).
 */
function updateEstateOwnerBadge(estateId, ownerName, ownerUuid) {
    const block = document.querySelector('.estate-block[data-estate-id="' + estateId + '"]');
    if (!block) return;

    const badge = block.querySelector('.estate-role-badge');
    if (badge) {
        badge.textContent = ownerName;
        badge.className = 'estate-role-badge ' + (ownerUuid === <?= json_encode(NULL_UUID) ?> ? 'manager' : 'owner');
    }
}

/* ── Extend Escape to cover the region modal, Estate Tools modal, the
       remove-manager confirm modal, and the Take Ownership confirm modal
       (checked in stacking order, topmost first, since several of these
       can be open on top of each other). The inline Change Owner section
       isn't a modal/overlay — its own search-results dropdown already
       handles Escape (see changeOwnerSearchInputEl's keydown listener
       above), so nothing extra is needed here for it. ─────────────────── */
document.addEventListener('keydown', function(e) {
    if (e.key !== 'Escape') return;

    if (document.getElementById('takeOwnershipModal').classList.contains('open')) {
        closeTakeOwnershipModal();
        return;
    }
